...
class FruitNameComparator implements Comparator<Fruit3> {
	public Comparator price2;

	public int compare(Fruit3 f1, Fruit3 f2) {
		return f1.name.compareTo(f2.name);
	//if (Integer.compare(f1.price,f2.price){
...
public class 실습3_7객체배열이진탐색 {
	...
	private static void sortData(Fruit3[] arr, Comparator<Fruit3> cc_price) { // 비교연산자
	...
	static void sortData(Fruit3[] arr, FruitNameComparator cc) { // 객체
		for (int i = 0; i < arr.length; i++) {
			for (int j = i + 1; j < arr.length; j++) {
	...
	public static void main(String[] args) {
		...
		Arrays.sort(arr, cc); // Fruit3 배열 , 비교연산자
		sortData(arr, cc);
		// showData(arr);
