...
public class Test01 {
	public static void main(String[] args) {
		...
		arr[0] = s;
		arr[1] = i1;
		//int len = arr[0].length();      		//Object 클래스에는 length()메서드가 없다
		int len = ((String) arr[0]).length();

		ArrayList lst = new ArrayList();  //Object 형으로 들어감
		String s2 = "java";
		Integer i2 = 123;
		lst.add(s2);
		lst.add(i2);
		...
		int len_string = list.get(0).length(); //list는 string 객체이다 그 차이는(원인은?) 제네릭 떄문
